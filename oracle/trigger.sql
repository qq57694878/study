  ...
  BEGIN
    CASE WHEN INSERTING THEN
      ...
      SELECT PART1||PART2 INTO V_SQL FROM
        (SELECT 'UPDATE   '||V_TABLE_NAME||' SET '|| WM_CONCAT(T1.COLUMN_NAME||'=:NEW.'||T1.COLUMN_NAME) as PART1  FROM USER_TAB_COLUMNS T1 WHERE T1.TABLE_NAME=V_TABLE_NAME) A,
        (select ' where '||replace(wm_concat(cu.column_name||'=:NEW.'||cu.column_name),',',' and ')||'' AS PART2 from user_cons_columns cu, user_constraints au where cu.constraint_name = au.constraint_name and au.constraint_type = 'P' and au.table_name = V_TABLE_NAME)B;
...
SELECT T1.COLUMN_NAME ,CASE WHEN EXISTS(SELECT 1 FROM   user_cons_columns A, user_constraints B WHERE A.constraint_name = B.constraint_name  AND B.constraint_type='P' AND A.column_name=T1.COLUMN_NAME AND A.table_name=T1.TABLE_NAME) THEN 1 ELSE 0 END PK_BJ
...
SELECT T1.COLUMN_NAME,DECODE(NVL(T2.column_name,'0'),'0',0,1) PK_BJ  FROM USER_TAB_COLUMNS T1
  LEFT JOIN (SELECT A.column_name,A.table_name FROM  user_cons_columns A, user_constraints B WHERE A.constraint_name = B.constraint_name  AND B.constraint_type='P'
            ) T2
